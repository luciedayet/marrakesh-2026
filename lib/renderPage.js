...
// Replaces the original inline PIN-check script: the PIN list must never
// be sent to the browser before (or without) a verified server-side
// login, so this only talks to /api/login.
...
// While a smooth scroll triggered by a click is in flight, the sections we
// pass through also cross the observer's threshold and would otherwise
// briefly steal the active state back before we land on the target.
...
function renderLoggedIn(user) {
  ...
  // The template ships this with a stray inline display:none that a
  // non-!important media query can never override; drop it so the existing
  // "@media (max-width:768px)" rules actually get to show the mobile top bar.
  $('#mobileTopbar').removeAttr('style');
